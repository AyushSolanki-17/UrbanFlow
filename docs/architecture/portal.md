...
Validate every payload before publication. Publish a complete release under a new version, then conditionally switch its manifest/pointer against the expected predecessor so readers never mix releases and an older job cannot overwrite a newer publication. A page pins one export ID for all payload reads; cache invalidation must not combine files from different releases. Retain a previous valid release for rollback within the disk budget. Display coverage and generated-at time; a publicly reachable page does not imply continuously arriving data. Export failure keeps the previous release visible with its original freshness metadata.
